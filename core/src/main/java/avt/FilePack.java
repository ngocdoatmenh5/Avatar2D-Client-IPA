...
public final class FilePack {
   ...
   public FilePack(String var1) {
      ...
      try {
         InputStream stream = CRes.getResourceAsStream(this.name);
         if (stream == null) {
            throw new IOException("FilePack not found: " + this.name);
         }
         this.file = new DataInputStream(stream);
   ...
   public static Image getImage(String var0) {
      ...
      if (img == null) {
         try {
            img = CRes.createImage(T.getPath() + "/" + var0 + ".png");
         } catch (Throwable t) {
         }
      }
   ...
   private Image loadImage(String var1) {
      for(int var2 = 0; var2 < this.nFile; ++var2) {
         if (this.fname[var2].compareTo(var1) == 0) {
            return CRes.createImage(this.fullData, this.fpos[var2], this.flen[var2]);
         }
      }
