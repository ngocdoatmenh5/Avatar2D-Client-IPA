...
public final class MediumPaint implements IPaint {
   ...
   public MediumPaint() {
      try {
         Canvas.imagePlug = CRes.createImage(T.getPath() + "/12Plus.png");
      ...
      try {
         MyScreen.ao = CRes.createImage(T.getPath() + "/on/msg0.on");
         RoomListOnScr.imgRoomStat = new FrameImage(CRes.createImage(T.getPath() + "/on/stat.on"), 11, 11);
      ...
      if (b == null) {
         try {
            ...
            for(var1 = 0; var1 < 14; ++var1) {
               f[var1] = CRes.createImage(T.getPath() + "/card/c" + var1 + ".png");
            }
            ...
            Image var3;
            Graphics var4;
            (var4 = (var3 = CRes.createImage(var1 << 1, var2 << 1)).getGraphics()).setColor(-523560);
            ...
            f[12] = var3;
            g[0] = new FrameImage(CRes.createImage(T.getPath() + "/card/f.png"), 8, 9);
            g[1] = new FrameImage(CRes.createImage(T.getPath() + "/card/g.png"), 8, 9);
            b = CRes.createImage(T.getPath() + "/card/cb.png");
            c = CRes.createImage(T.getPath() + "/card/cb1.png");
            d = CRes.createImage(T.getPath() + "/card/cb2.png");
   ...
   public final void paintDefaultBg(Graphics var1) {
      int bgOffsetY = 42 * AvMain.hd;
      for(int var2 = 0; var2 < Canvas.w / 50 + 1; ++var2) {
         for(int var3 = 0; var3 < Canvas.hCan / 71 + 2; ++var3) {
            int yFromBottom = Canvas.hCan - (var3 + 1) * 71 + bgOffsetY;
            var1.drawImage(e, var2 * 50, yFromBottom, 0);
         }
      }
   ...
   public final void setValue(int var1) {
      try {
         if (var1 == 0) {
            BoardListOnScr.imgBoard = new FrameImage(CRes.createImage(T.getPath() + "/on/imgBan2.on"), 60, 46);
         } else if (var1 == 1) {
            BoardListOnScr.imgBoard = new FrameImage(CRes.createImage(T.getPath() + "/on/imgBan4.on"), 60, 46);
         } else {
            BoardListOnScr.imgBoard = new FrameImage(CRes.createImage(T.getPath() + "/on/imgBan5.on"), 60, 46);
         }
      } catch (Exception var3) {
         var3.printStackTrace();
      }
   ...
   public final void initResourceOne() {
      try {
         ...
         a = new FrameImage(CRes.createImage(T.getPath() + "/on/round.on"), 8, 8);
         e = CRes.createImage(T.getPath() + "/on/bg.on");
         j = new Image[8];

         for(int var1 = 0; var1 < 8; ++var1) {
            j[var1] = CRes.createImage(T.getPath() + "/on/imgPopup" + var1 + ".on");
         }

         l = new FrameImage(CRes.createImage(T.getPath() + "/barMoney.png"), 10, 10);
      } catch (Exception var2) {
         var2.printStackTrace();
      }
   ...
   public final void initResourceThree() {
      try {
         k = new FrameImage(CRes.createImage(T.getPath() + "/on/imgDoor.on"), 45, 44);
         new FrameImage(CRes.createImage(T.getPath() + "/on/trangthai.on"), 11, 4);
         BoardListOnScr.imgSelectBoard = CRes.createImage(T.getPath() + "/on/imgSelectban.on");
      } catch (Exception var2) {
         var2.printStackTrace();
      }
