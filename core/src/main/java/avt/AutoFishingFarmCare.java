...
public final class AutoFishingFarmCare {
   ...
   public static boolean tick() {
      ...
      if (!started) {
         log("start quick care direct typemap=" + LoadMap.TYPEMAP + " screen=" + main.Canvas.currentMyScreen);
         try {
            FarmScr.gI().startAutoFishingQuickCare();
            log("startAutoFishingQuickCare() invoked");
         } catch (Throwable t) {
            log("startAutoFishingQuickCare failed: " + t);
         }
         started = true;
